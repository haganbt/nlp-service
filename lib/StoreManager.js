	...
	try {
    	...
    	if (body.hasOwnProperty(i)) {
  			...
  			var item = JSON.parse(body[i]);
				...
				if (item.interaction && item.interaction.content && item.interaction.tag_tree && item.interaction.tag_tree.classifier && item.interaction.tag_tree.classifier.train && item.interaction.tag_tree.classifier.train.length === 1){
